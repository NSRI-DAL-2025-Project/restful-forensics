...
bcf_to_vcf <- function(input.file, output.dir){
   output_file <- file.path(output.dir, "bcftovcf")
...
plink_to_vcf <- function(bed.file, bim.file, fam.file, output.dir){
   output_file <- file.path(output.dir, "plinktovcf")
...
vcf_to_csv <- function(files, ref = NULL, output.dir = output.dir) {
   library(dplyr)
   library(purrr)
...
get_colors_labels <- function(fsnps_gen, use_default, input_labels = NULL, input_colors = NULL, input_shapes = NULL) {
   library(ade4)
   library(adegenet)
...
plot_pca <- function(ind_coords, centroid, percent, labels_colors, width = 8, height = 8, pc_x = 1, pc_y = 2) {
   ...
   plot <- ggplot(ind_coords, aes(x = ind_coords[[paste0("PC", pc_x)]],
      ...
      geom_point(shape = Site, size = 3, show.legend = FALSE) + # shape is changed
...
structureRead <- function(file,
   ...
   q.mat <- q.mat[order(q.mat$id), ] 
   # removed q.mat$row since row is nonexistent
   #q.mat$row <- NULL
   rownames(q.mat) <- NULL
   ...
   # Removed from original function
